...
const SupportDetails = () => {
    ...
    const handleSendComment = async () => {
        ...
        await fetch('http://localhost:5000/api/messages', {
            ...
            body: JSON.stringify({
                user: adminUser.name,
                userEmail: state.email || 'unknown@example.com',
                topic: state.topic || state.description || 'Без темы',
                type: 'Поддержка',
                text: comment,
            }),
        });
    ...
    return (
        <div className="support-details">
            ...
            <h2 className="title">Детали поддержки</h2>

            <h3>{state.name || 'Аноним'}</h3>
            <p><strong>Тема:</strong> {state.topic || 'Поддержка'}</p>
            <p><strong>Дата:</strong> {state.date}</p>
            <p><strong>Текст:</strong> {state.description || 'Нет текста'}</p>
